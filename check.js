...
const MOCK_MATCHES = [
        ...
        window.openPointModal = function(loai_diem) {
            window.ptEntryState = {
                loai_diem: loai_diem,
                tinh_chat: null,
                nguoi_ket_thuc: null,
        ...
        window.renderPtEntryForm = function() { let currentScroll = 0; const scrollEl = document.getElementById('pt-entry-scroll'); if (scrollEl) currentScroll = scrollEl.scrollTop; const s = window.ptEntryState;
            const match = appState.matches.find(m => m.id_tran_dau === appState.currentMatchId);
            const me = match.thong_tin.doi_thu_1 || 'Tôi';
            const opp = match.thong_tin.doi_thu_2 || 'Đối thủ';
            ...
            let html = `<div id="pt-entry-scroll" class="max-h-[70vh] overflow-y-auto no-scrollbar pb-6 px-1">
        ...
        window.openEditPointModal = function(idx) {
            const match = appState.matches.find(m => m.id_tran_dau === appState.currentMatchId);
            const game = match.chi_tiet_game[appState.currentGameIndex];
        ...
        // --- DASHBOARD ---
function calculateDashboardStats(filteredMatches, perspective = 'doi_thu_1') {
    const s = {
...
function renderDashboard(container) {
        ...
        <div>
            ...
            <div class="bg-white rounded-xl shadow-sm border p-4 mb-4">
                ...
                <div class="grid grid-cols-3 gap-2 text-center mb-4">
                    ...
                    <div><div class="text-[10px] uppercase text-gray-500 font-bold">Bị ép nên tự đánh hỏng</div><div class="font-black text-red-600 text-2xl">${s.unforcedErrorByMe}</div></div>
                </div>
            </div>
        </div>
    `;
    ...
    html += `<div class="bg-white rounded-xl shadow-sm border p-4 mb-4"><h3 class="font-bold text-gray-800 border-b pb-2 mb-3">Ép đối thủ đánh hỏng</h3>`;
    ...
    html += `<div class="bg-white rounded-xl shadow-sm border p-4 mb-4"><h3 class="font-bold text-gray-800 border-b pb-2 mb-3">Bị ép nên tự đánh hỏng</h3>`;
    ...
    // Heatmap
    const rHeatmap = (data, title, overlayColorClass, reverseCols = false, reverseRows = false) => {
    ...
    html += `<div class="bg-white rounded-xl shadow-sm border p-4 mb-4"><h3 class="font-bold text-gray-800 border-b pb-2 mb-4">ĐIỂM RƠI (HEATMAP)</h3><div class="flex gap-4">${rHeatmap(s.heatmapWinner, 'GHI ĐIỂM (WINNER)', 'bg-blue-600', true, false)}${rHeatmap(s.heatmapLost, 'MẤT ĐIỂM (LỖI)', 'bg-red-600', false, true)}</div></div>`;
